...
- `game_scores` 컬렉션에 약 30만 건이 쌓이면서, **랭킹용 집계**가 매번 전체를 `email`+`createdAt` 정렬 후 `$group`으로 처리해 460~720ms 수준의 지연 발생.
...
- **위치**: `src/lib/models/slotUserBalance.js`  
  필드: `email`(unique), `nickname`, `balance`, `totalSpin`, `updatedAt`.
- **인덱스**: `{ balance: -1 }` (랭킹 조회), `{ email: 1 }` unique.
...
- **이전**: `game_scores` 전체 `$sort` + `$group` (수십만 건 스캔).
- **이후**: `SlotUserBalance.find({ totalSpin: { $gt: 0 } }).sort({ balance: -1 }).limit(10)`  
  → 소량 문서만 조회해 응답 시간 단축.

### 4. getTodaySlotStats 인덱스

- `game_scores`에 `{ game: 1, createdAt: -1 }` 인덱스 추가.  
