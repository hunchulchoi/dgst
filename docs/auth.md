...
- **로컬 개발**: `.env`에 `REDIS_URL=redis://redis:6379` 또는 `redis://dgst_redis:6379` (호스트 실행 시 127.0.0.1로 자동 보정)
- **Docker·운영**: compose **서비스명** `redis`로 DNS 조회 (`container_name` `dgst_redis`와 다름). `dgst_svelte`에 `REDIS_URL=redis://redis:6379`
