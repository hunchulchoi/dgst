<script>
  ...
  function checkRecaptcha(){
    grecaptcha.ready(function () {
      grecaptcha.execute(PUBLIC_GOOGLE_RECAPTCHA_SITE_KEY, {action: 'register'})
  ...
  const changeHandler = async (target)=>{
    ...
    switch (target.id){
      case 'nickname':
        ...
        if(!invalids.nickname){
          ...
          fetch(`/auth/register/${target.value}`)
                  .then(res=>{
                    if(res.status !== 204){
                      alert('사용중인 아이디 입니다.');
    ...
    console.log('nickname', nickname,'invalids', invalids.nickname, 'introduction', introduction, 'invalids', invalids.introduction, 'fight', fight);
  }

  $: isInvalid = !(nickname && !invalids.nickname && introduction && !invalids.introduction && fight);
  $: console.log(isInvalid)

</script>

<svelte:head>
  <script src="https://www.google.com/recaptcha/api.js?render={PUBLIC_GOOGLE_RECAPTCHA_SITE_KEY}"></script>
</svelte:head>
        ...
        <Card class="p-2">
          <Row>
            <Col xs="4" md="3" class="d-flex align-items-center">
              <Image id="preview" src="/icons/unknown-person-icon-4.jpg" thumbnail width="100" height="100" alt="프로필 사진" class="card-img-left rounded-2"/>
            </Col>
            <Col xs="7" md="8">
              <CardBody>
        ...
        <Form>
          ...
          <FormGroup floating label="닉네임">
            <Input id="nickname"
                   ...
                   class="form-control needs-validation" autofocus maxlength="15"/>
          </FormGroup>
          <InputGroup class="mb-3">
            <InputGroupText><Icon name="image me-2"/>사진</InputGroupText>
            <input id="photo" type="file" on:change="{(evt)=>preview(evt.target)}" class="form-control" accept="image/*"/>
          </InputGroup>
          ...
          <FormGroup>
            ...
            <FormCheck id="fight"
                       ...
                       on:change={evt=>changeHandler(evt.target)}
                       feedback="체크 해 주세요"
                       size="lg" class="needs-validation"/>
            <Popover
                trigger="hover"
                placement="top"
                target="fight"
          ...
          <hr/>
          <div class="text-end">
            <Button size="lg" on:click={doSubmit} color="success"
                    bind:disabled={isInvalid}>
              <Icon name="arrow-through-heart-fill pe-2"></Icon>가입
            </Button>
          </div>
        </Form>
      </Row>
