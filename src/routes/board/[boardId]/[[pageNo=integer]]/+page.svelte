<script>
  import { Badge, Button, Col, Icon, Image, Pagination, PaginationItem, PaginationLink, Row, Table } from "sveltestrap";
...
<main class="container p-4 my-4">
  ...
  {:else}
    {#each data.articles as article}
        <tr>
          <th scope="row" style="cursor:pointer" on:click={()=>read(article._id)} class="text-break">
            {article.title}
          ...
          <td class="text-muted text-end"><Icon name="hand-thumbs-up" class="text-success pe-1"/>{article.like}</td>
    ...
    <Row class="mt-3">
      <Col md="10">
        <Pagination size="md" arialabel="페이지 네이션" class="d-flex justify-content-center">
          <PaginationItem><PaginationLink first href="{`/board/${$page.params.boardId}`}"/></PaginationItem>
        {#each Array(data.maxPage) as _, i}
          <PaginationItem active={((!$page.params.pageNo) && i===0) || (i+1)==$page.params.pageNo}>
            <PaginationLink href="{`/board/${$page.params.boardId}/${i+1}`}">
              {i+1}
            </PaginationLink>
          </PaginationItem>
        {/each}
          <PaginationItem><PaginationLink last href={`/board/${$page.params.boardId}/${data.maxPage}`}/></PaginationItem>
        </Pagination>
      </Col>
      {#if $page.data.session?.user.nickname}
      <Col md="2" class="text-end">
        <Button color="primary" on:click={write}><Icon name="pencil-fill" class="pe-2"/>글쓰기</Button>
      </Col>
      {/if}
    </Row>
  </Row>
